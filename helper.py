...
def process_spec_file(
    file_path: str = None, token: str = None, key: str = None, username: str = None
):
...
def replace_api_credentials(model, scenario, actual_key, actual_token):
    ...
    updated_content = [
        line.replace(r"{key}", actual_key).replace(r"{token}", actual_token)
        for line in content
    ]
    ...
    def replace_values(d, actual_key, actual_token):
        for key, value in d.items():
            if isinstance(value, dict):
                ...
                d[key] = value.replace(r"{key}", actual_key).replace(
                    r"{token}", actual_token
                )
    ...
    def update_placeholders(obj):
        if isinstance(obj, list):
            process_list_of_dicts(
                lst=obj, actual_key=actual_key, actual_token=actual_token
            )
...
def replace_api_credentials_in_jira_json(
    scenario, actual_token, actual_host, actual_username
):
    # Open the JSON file and load the content
    with open(f"./specs/{scenario}_base.json", "r") as json_file:
        content = json.load(json_file)
    ...
    def replace_values(
        d, actual_token, actual_host, actual_username, actual_basepath=""
    ):
        for key, value in d.items():
            if isinstance(value, dict):
                ...
                d[key] = value.replace(r"{{host}}", actual_host).replace(
                    r"{{basePath}}", actual_basepath
                )
            elif isinstance(value, list):
                try:
                    d[key] = [
                        item.replace(r"{{username}}", actual_username).replace(
                            r"{{apiToken}}", actual_token
                        )
                        for item in value
                    ]
    ...
    def update_placeholders(obj):
        if isinstance(obj, list):
            try:
                obj = [
                    item.replace(r"{{username}}", actual_username)
                    .replace(r"{{apiToken}}", actual_token)
                    .replace(r"{{host}}", actual_host)
                    .replace(r"{{basePath}}", "")
                    for item in obj
                ]
            except Exception as e:
...
def replace_credentials_salesforce_json(
    scenario,
    actual_domain,
    actual_version,
    actual_client_id,
